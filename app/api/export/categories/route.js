...
import { exportCategories } from '@/lib/exportService';
...
async function handler(request) {
  ...
  try {
    if (format === 'pdf') {
      const doc = await exportCategories(user.companyId, projectId, 'pdf');
      ...
      const csvContent = await exportCategories(user.companyId, projectId, 'csv');
