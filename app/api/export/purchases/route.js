...
import { exportPurchases } from '@/lib/exportService';
...
async function handler(request) {
  ...
  try {
    if (format === 'pdf') {
      const doc = await exportPurchases(user.companyId, projectId, 'pdf');
      ...
      const csvContent = await exportPurchases(user.companyId, projectId, 'csv');
